...
if(instance_exists(player_reference)){
	...
	var target_x = player_reference.x + ((follow_xdistance-500) *side);
...
if (!instance_exists(target) || (instance_exists(target) && target.is_dead)){
...
//Upgraded Portion
	...
	x = lerp(x, target_x, follow_speed);
    y = lerp(y, target_y, follow_speed);
	image_xscale =-1* player_reference.image_xscale;
}
...
if (target != noone && !target.is_dead && point_distance(x, y, target.x, target.y) <= attack_range)
{
	...
	if (!is_attacking)
	{
		if (sprite_index != spr_drone_upgrade_idle)
		{
			sprite_index = spr_drone_upgrade_idle;
			image_index = 0;
		}
	}
