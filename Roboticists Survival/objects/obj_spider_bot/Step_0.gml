...
if(enemy_hp<=0 && !is_dead){
	is_dead=true
	sprite_index=spr_spider_bot_death
	image_index=0;
	image_speed=1
	//Small innate xp gain
	obj_player.experience+=5;
	audio_play_sound(snd_spider_bot_death,0,false);
...
if (instance_exists(obj_player))
{
	...
	if (distance_to_object(obj_player) < attack_range)
	{
		...
		if (distance_to_object(obj_player) <= melee_range && can_attack && !is_attacking)
		{
			...
			sprite_index = spr_spider_bot_attack;
			image_index = 0;
			movement_speed=0;
...
if (!is_attacking)
{
	if (sprite_index != spr_spider_bot_run)
	{
		sprite_index = spr_spider_bot_run;
		image_index = 0;
	}
}
...
if (is_attacking)
{
	if (sprite_index == spr_spider_bot_attack && image_index >= image_number - 1)
	{
		is_attacking = false;
